...
// Fix for TypeScript EyeDropper error
declare global {
  interface Window {
    EyeDropper: any;
  }
...
// UPDATED: Added White (#FFFFFF) and Near-Black (#0A0A0A) at the start
const PRESET_COLORS = [
...
export const SettingsStep: React.FC<SettingsStepProps> = ({
  ...
  const [expertPromptInput, setExpertPromptInput] = useState(
    config.expertPrompt || ''
  );
  ...
  const handleRetouchChange = (
    retouchLevel: GenerationConfig['retouchLevel']
  ) => updateConfig({ retouchLevel });

  const handleBrandColorChange = (color: string) =>
    updateConfig({ brandColor: color });

  const handleSecondaryBrandColorChange = (color: string) =>
    updateConfig({ secondaryBrandColor: color });

  const handleKeepGlassesChange = (keep: boolean) =>
    updateConfig({ keepGlasses: keep });

  const isEyeDropperSupported = !!window.EyeDropper;
  const pickColorWithEyeDropper = async (
    field: 'brandColor' | 'secondaryBrandColor' | 'customBg'
  ) => {
    ...
    try {
      ...
      if (field === 'brandColor') handleBrandColorChange(hexColor);
          ...
          // CRITICAL FIX: Update state AND prompt directly
  ...
  const handleVariationChange = (level: VariationLevel) => {
    setVariationLevel(level);
  };

  const handleBodyOffsetChange = (offset: number) => {
    if (offset < -3 || offset > 3) return;
    setBodySizeOffset(offset);
  };
  ...
  const handleLightingChange = (lighting: string) => updateConfig({ lighting });
  ...
  const handleSurpriseMe = () => {
    if (!clothingStyleGroup) {
      alert('Choose a Clothing Style first, then click Surprise Me.');
      return;
    }
    ...
    const clothingChoice =
      allClothingChoices[
        Math.floor(Math.random() * allClothingChoices.length)
      ];
  ...
  const handleAddOrUpdateLook = () => {
    if (!canBuildLook) {
      alert(
        'Please choose a Clothing Style, clothing option, scene, and camera settings for this Look.'
      );
      return;
    }
    ...
    const newLook: LookConfig = {
      ...
      label: activeLookId
        ? looks.find((l) => l.id === activeLookId)?.label || 'Look'
        : `Look #${looks.length + 1}`,
      clothingStyleGroup,
      clothingOption,
      sceneId,
    ...
    setLooks((prev) => {
      if (activeLookId) {
        return prev.map((look) =>
          look.id === activeLookId ? newLook : look
        );
      }
      if (prev.length >= MAX_LOOKS) {
        alert(`You can create up to ${MAX_LOOKS} Looks.`);
        return prev;
      }
      return [...prev, newLook];
    });
  ...
  const handleEditLook = (look: LookConfig) => {
    ...
    setBodySizeOffset(
      typeof look.bodySizeOffset === 'number'
        ? look.bodySizeOffset
        : DEFAULT_BODY_OFFSET
    );
    onChange(cloneConfig(look.config));
    
    // Reset custom toggles for editing unless we specifically detect custom values later (simplified)
    setIsCustomClothing(false);
    setIsCustomBackground(false);
  };
  ...
  const handleContinue = () => {
    ...
    if (creationMode === 'expert' && expertPromptInput.trim()) {
      ...
      if (looks.length === 0) {
        alert(
          'Please create at least one Look, or switch to Expert mode and provide a custom prompt.'
        );
        return;
      }
  ...
  // Helper to render the Color Picker Popup
  const renderColorPicker = (type: 'primary' | 'secondary' | 'customBg', currentValue: string, onChange: (val: string) => void, placeholder: string) => {
    ...
    return (
      <div className="relative">
        <button
          ...
          className={`w-full flex items-center gap-2 bg-slate-950 border rounded-lg px-3 py-2 text-xs transition-all ${
            isActive ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-slate-700 hover:border-slate-500'
          }`}
        >
          <div 
          ...
          />
          <span className={currentValue ? 'text-white' : 'text-slate-500'}>
        ...
        {isActive && (
          <div 
            ref={colorPickerRef}
            className="absolute bottom-full left-0 mb-2 z-50 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-4 w-64 animate-fade-in"
          >
            <button
              type="button"
              onClick={() => {
              ...
              className="flex items-center gap-2 w-full text-left px-2 py-1.5 mb-3 text-xs text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors border border-transparent hover:border-slate-700"
            >
            ...
            <div className="grid grid-cols-6 gap-2 mb-4">
              {PRESET_COLORS.map((color) => (
                <button
                  ...
                  className="w-8 h-8 rounded-full border border-slate-700 hover:scale-110 transition-transform relative group"
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="HEX"
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-5 pr-2 py-1.5 text-xs text-white focus:ring-1 focus:ring-indigo-500 outline-none"
                />
              </div>
              <button
                type="button"
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm max-w-lg">
            Create different "Looks" by mixing clothing styles and scenes.
            You can generate multiple images per Look.
          </p>
        </div>
          ...
          {creationMode === 'guided' ? (
            <>
              ...
              <section className="bg-slate-950/70 border border-slate-800 rounded-2xl p-6 sm:p-7 shadow-inner relative overflow-hidden">
                <div className="flex items-center justify-between mb-4 relative z-10">
                  ...
                  {clothingStyleGroup && (
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs text-indigo-300 hover:text-indigo-200 transition-colors bg-indigo-500/10 px-3 py-1.5 rounded-full border border-indigo-500/20 hover:border-indigo-500/50"
                    >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative z-10">
                  {Object.entries(BRAND_DEFINITIONS).map(([key, brand]) => {
                    ...
                    const Icon = brand.icon || Briefcase;
                    return (
                      <button
                        key={key}
                        type="button"
                        onClick={() => handleClothingStyleGroupSelect(key)}
                        className={`relative flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-200 group ${
                          isActive
                            ? 'bg-indigo-600 border-indigo-500 shadow-xl shadow-indigo-900/20'
                        ...
                        <div
                          className={`mb-3 p-2.5 rounded-full ${
                            isActive
                              ...
                              : 'bg-slate-800 text-slate-400 group-hover:text-indigo-300 group-hover:bg-slate-700'
                          }`}
                        >
                  ...
                  {!isCustomClothing ? (
                    <>
                      ...
                      <div className="mt-6 pt-4 border-t border-slate-800">
                        ...
                        <button 
                          onClick={handleCustomClothingToggle}
                          className="w-full py-3 rounded-xl border border-dashed border-slate-700 hover:border-indigo-500 hover:bg-slate-800/50 text-slate-400 hover:text-white transition-all flex items-center justify-center gap-2 text-xs font-medium"
                        >
                    ...
                    <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700">
                      ...
                      <textarea 
                        value={customClothingText}
                        onChange={handleCustomClothingTextChange}
                        placeholder="Describe the outfit in detail (e.g., 'A vintage 1970s suede jacket over a white turtleneck')..."
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-xs text-white min-h-[80px]"
                      />
                    </div>
                  )}
                </section>
                  ...
                  {!isCustomBackground ? (
                    <>
                      ...
                      <div className="mt-6 pt-4 border-t border-slate-800">
                        ...
                        <button 
                          onClick={handleCustomBackgroundToggle}
                          className="w-full py-3 rounded-xl border border-dashed border-slate-700 hover:border-indigo-500 hover:bg-slate-800/50 text-slate-400 hover:text-white transition-all flex items-center justify-center gap-2 text-xs font-medium"
                        >
                          <Plus className="w-4 h-4" />
                    ...
                    <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700">
                      ...
                      <div className="flex gap-2 mb-4 bg-slate-950 p-1 rounded-lg w-fit">
                        <button 
                          onClick={() => { setCustomBgMode('prompt'); updateCustomBackground('prompt'); }}
                          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${customBgMode === 'prompt' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                        >
                          Describe
                        </button>
                        <button 
                          onClick={() => { setCustomBgMode('color'); updateCustomBackground('color'); }}
                          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${customBgMode === 'color' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                        >
                          Color
                        </button>
                        <button 
                          onClick={() => { setCustomBgMode('upload'); updateCustomBackground('upload'); }}
                          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${customBgMode === 'upload' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                        >
                          Upload
                        </button>
                      </div>
                      ...
                      {customBgMode === 'prompt' && (
                        <div className="space-y-2">
                          <label className="text-[10px] text-slate-500 uppercase font-bold">Background Description</label>
                          <textarea 
                            value={customBgText}
                            onChange={(e) => { setCustomBgText(e.target.value); setScenePrompt(e.target.value); updateConfig({ backgroundType: e.target.value }); }}
                            placeholder="e.g. A futuristic mars colony interior..."
                            className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-xs text-white min-h-[80px]"
                          />
                        </div>
                      )}
                      ...
                      {customBgMode === 'color' && (
                        <div className="space-y-2">
                          <label className="text-[10px] text-slate-500 uppercase font-bold">Solid Background Color</label>
                          <div className="w-1/2">
                      ...
                      {customBgMode === 'upload' && (
                        <div className="space-y-3">
                          <label className="text-[10px] text-slate-500 uppercase font-bold">Upload Background Image</label>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <Maximize className="w-4 h-4 text-indigo-300" />
                        <p className="text-[11px] uppercase tracking-wide" style={{ color: ORANGE }}>
                          Aspect Ratio
                        </p>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        {['1:1', '16:9', '9:16', '4:5'].map((ratio) => (
                          <button
                            ...
                            className={`text-[11px] rounded-lg px-2.5 py-1.5 transition ${
                              (config.aspectRatio || '1:1') === ratio
                                ? 'border-2 border-indigo-500 bg-indigo-500/10 text-white'
                                : 'border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-indigo-400/60'
                            }`}
                          >
                            {ratio}
                          </button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['Polished Professional', 'Daylight', 'Cinematic', 'Dark & Moody'].map((m) => (
                          <button
                            ...
                            className={`text-[11px] rounded-lg px-2.5 py-1.5 transition ${
                              (config.mood || 'Polished Professional') === m
                                ? 'border-2 border-indigo-500 bg-indigo-500/10 text-white'
                                : 'border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-indigo-400/60'
                            }`}
                          >
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['Eye Level', 'High Angle', 'Low Angle (Power)'].map((angle) => (
                          <button
                            key={angle}
                            type="button"
                            onClick={() => handleCameraAngleChange(angle)}
                            className={`text-[11px] rounded-lg px-2.5 py-1.5 transition ${
                              (config.cameraAngle || 'Eye Level') === angle
                                ? 'border-2 border-indigo-500 bg-indigo-500/10 text-white'
                                : 'border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-indigo-400/60'
                            }`}
                          >
              ...
              <section className="space-y-3">
                {/* Retouch / Brand colors */}
                <div className="grid md:grid-cols-2 gap-3">
                  {/* Retouch box */}
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                      <Droplet className="w-4 h-4 text-indigo-300" />
                      <p
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {['None', 'Natural', 'Polished', 'Airbrushed'].map(
                        (r) => (
                          <button
                            key={r}
                            type="button"
                            onClick={() => handleRetouchChange(r as any)}
                            className={`text-[11px] rounded-lg px-2.5 py-1.5 transition ${
                              (config.retouchLevel || 'None') === r
                                ? 'border-2 border-indigo-500 bg-indigo-500/10 text-white'
                                : 'border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-indigo-400/60 hover:bg-slate-900'
                            }`}
                          >
                            {r}
                          </button>
                  ...
                  {/* Brand Colors - UPDATED WITH PALETTE PICKER */}
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                    ...
                    {/* Row 1: Colors (Primary + Secondary) */}
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                      ...
                      <div>
                        ...
                        {renderColorPicker(
                          'secondary', 
                          config.secondaryBrandColor || '', 
                          handleSecondaryBrandColorChange, 
                          'Choose Secondary'
                        )}
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p
                        ...
                        style={{ color: ORANGE }}
                      >
                        Variation for this Look
                      </p>
                    </div>
                    <p className="text-[11px] text-slate-400 mb-1">
                      Controls how different each image will be while keeping
                      the same mood and lighting.
                    </p>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { label: 'Low', value: 'low' as VariationLevel },
                        { label: 'Medium', value: 'medium' as VariationLevel },
                        { label: 'High', value: 'high' as VariationLevel },
                      ].map((opt) => (
                        <button
                          key={opt.value}
                          type="button"
                          onClick={() => handleVariationChange(opt.value)}
                          className={`text-[11px] rounded-lg px-2.5 py-1.5 transition ${
                  ...
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <p
                        ...
                        style={{ color: ORANGE }}
                      >
                        Body size for this Look
                      </p>
                    </div>
                    <p className="text-[11px] text-slate-400 mb-1">
                      Adjusts how slim or full the body appears. 0 = same as
                      reference photo.
                    </p>
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        max={3}
                        step={1}
                        value={bodySizeOffset}
                        onChange={(e) =>
                          handleBodyOffsetChange(parseInt(e.target.value, 10))
                        }
                    ...
                    <div className="flex justify-between text-[11px] text-slate-400">
                      ...
                      <button
                        type="button"
                        onClick={() => handleBodyOffsetChange(0)}
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                      <Glasses className="w-4 h-4 text-indigo-300" />
                      <p
                        className="text-[11px] uppercase tracking-wide"
                        style={{ color: ORANGE }}
                      >
                    ...
                    <div className="flex gap-2 mb-1">
                      <button
                        type="button"
                        onClick={() => handleKeepGlassesChange(true)}
                        className={`flex-1 text-[11px] rounded-lg px-3 py-2 transition ${
                          config.keepGlasses !== false
                            ? 'border-2 border-indigo-500 bg-indigo-500/10 text-white'
                            : 'border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-indigo-400/60 hover:bg-slate-900'
                        }`}
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => handleKeepGlassesChange(false)}
                        className={`flex-1 text-[11px] rounded-lg px-3 py-2 transition ${
                          config.keepGlasses === false
                            ? 'border-2 border-indigo-500 bg-indigo-500/10 text-white'
                            : 'border border-slate-700 bg-slate-900/60 text-slate-200 hover:border-indigo-400/60 hover:bg-slate-900'
                  ...
                  {/* Images for this Look box - UPDATED TO SLIDER */}
                  <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-1">
                      <ImageIcon className="w-4 h-4 text-indigo-300" />
                      <p
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="flex-1 accent-indigo-500"
                      />
                      <div className="w-8 text-center bg-slate-800 rounded px-1.5 py-0.5 text-xs font-bold text-white border border-slate-600">
                        {imageCount}
                      </div>
                    </div>
                    <p className="text-[10px] text-slate-500">
                      Generating {imageCount} unique variations.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="text-[11px] text-slate-400">
                    Configure this Look, then click{' '}
            ...
            <section className="bg-slate-950/70 border border-slate-800 rounded-2xl p-6 sm:p-7 shadow-inner">
              ...
              <p className="text-xs mb-3" style={{ color: ORANGE }}>
                Describe exactly what you want. The prompt will be applied to
                all images.
              </p>
              <textarea
                value={expertPromptInput}
                onChange={(e) => handleExpertPromptChange(e.target.value)}
                rows={8}
                placeholder="Describe lighting, mood, background, clothing, and any other details you care about..."
                className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white placeholder:text-slate-600 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              />
            </section>
          )}
        </div>

        {/* RIGHT COLUMN: LOOKS LIST & SUMMARY */}
        <div className="space-y-6">
          {/* Looks list */}
          <section className="bg-slate-950/70 border border-slate-800 rounded-2xl p-6 shadow-inner">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-white">
                Saved Looks ({looks.length}/{MAX_LOOKS})
              </h3>
            </div>

            {looks.length === 0 ? (
              <p className="text-xs text-slate-500">
                No Looks created yet. Configure a Look on the left, then click
                “Save Look”.
              </p>
            ) : (
              <div className="space-y-3">
                {looks.map((look) => (
                  <div
                    key={look.id}
                    className={`border rounded-xl px-3 py-2 text-xs flex items-center justify-between gap-3 ${
                  ...
                  >
                    <div>
                      ...
                      <p className="text-slate-300 mb-0.5">
                        {look.clothingOption} – {look.sceneName}
                      </p>
                      <p className="text-[11px] text-slate-500">
                        {look.imageCount} image{look.imageCount > 1 ? 's' : ''}{' '}
                        · {look.config.framing}, {look.config.aspectRatio} ·{' '}
                        {look.config.mood}, {look.config.lighting} · Variation:{' '}
                        {look.variationLevel || DEFAULT_VARIATION_LEVEL} · Body:{' '}
                        {typeof look.bodySizeOffset === 'number'
                          ? look.bodySizeOffset
                          : DEFAULT_BODY_OFFSET}
                      </p>
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="p-1.5 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-200"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-full bg-slate-800 hover:bg-red-600/80 text-slate-200"
                      >
          ...
          <section className="bg-slate-950/70 border border-slate-800 rounded-2xl p-6 shadow-inner space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-1">
                <p className="text-xs text-slate-400">
                  ...
                  <span className="text-indigo-300 font-semibold">
                    {creationMode === 'guided' ? totalImages : 1}
                  </span>
                </p>
                <p className="text-xs text-slate-400">
                  Mode:{' '}
            ...
            <div className="flex items-center justify-between">
              <Button
                variant="outline"
                onClick={onBack}
                className="border-slate-600 hover:bg-slate-800/50 text-xs"
              >
                Back
              </Button>
              <Button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 text-xs disabled:opacity-50"
              >
                Continue
              </Button>
            </div>
