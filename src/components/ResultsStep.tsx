            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              {displayImages.map((img, idx) => (
                <button key={img.id} type="button" onClick={() => setSelectedImage(img)}
                  ...
                  {img.styleName?.includes('(Edited)') && <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: T.purpleDim, backdropFilter: 'blur(4px)', color: T.purple, fontSize: 9, fontWeight: 700, textAlign: 'center', padding: '3px 0', letterSpacing: '0.06em' }}>EDITED</div>}
                  {img.styleName?.includes('(Regenerated)') && <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(76,29,149,0.6)', backdropFilter: 'blur(4px)', color: T.white80, fontSize: 9, fontWeight: 700, textAlign: 'center', padding: '3px 0', letterSpacing: '0.06em' }}>REGEN</div>}
                  {img.styleName?.includes('(Erased)') && <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: T.tealDim, backdropFilter: 'blur(4px)', color: T.teal, fontSize: 9, fontWeight: 700, textAlign: 'center', padding: '3px 0', letterSpacing: '0.06em' }}>ERASED</div>}
                </button>
              ))}
            </div>
          </div>
