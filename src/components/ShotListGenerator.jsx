...
const GEMINI_API_KEY = ""; // Will call via Anthropic proxy

// ─── Prompt ──────────────────────────────────────────────────────────────────
const buildShotListPrompt = (description) => `
...
// ─── Shot card ────────────────────────────────────────────────────────────────
const ShotCard = ({ shot, index, onUsePrompt }) => {
  const [expanded, setExpanded] = useState(false);
  const mood = MOOD_STYLES[shot.mood] || MOOD_STYLES["Expert"];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{
          color: "#475569", fontSize: 18, transform: expanded ? "rotate(180deg)" : "rotate(0deg)",
      ...
      {expanded && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: "#818cf8", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 4px 0" }}>
              Why this shot
            </p>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 6px 0" }}>
              Generation prompt
            </p>
            <p style={{ fontSize: 12, color: "#94a3b8", margin: 0, lineHeight: 1.7, fontFamily: "monospace" }}>
              {shot.prompt}
            </p>
          </div>
...
export default function ShotListGenerator() {
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: buildShotListPrompt(trimmed),
          }],
        }),
      });
  ...
  return (
    <>
      ...
      <div style={{
        minHeight: "100vh",
        background: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(79,70,229,0.18) 0%, transparent 60%), #080812",
        fontFamily: "'DM Sans', sans-serif",
        color: "#f1f5f9",
        ...
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 48, animation: "fadeIn 0.6s ease both" }}>
            ...
            <p style={{ fontSize: 16, color: "#64748b", margin: 0, lineHeight: 1.6 }}>
              Tell us what you do. We'll build a tailored photography plan<br />
              for your business — 10 specific images, ready to generate.
            </p>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12 }}>
              ...
              >
                {loading ? (
                  <><LoadingDots /> Building your shot list…</>
                ) : (
                  <>✦ Generate Shot List</>
                )}
              </button>
          ...
          {shots.length > 0 && (
            <div style={{ animation: "fadeIn 0.4s ease both" }}>
              ...
              <div style={{
                ...
                <div>
                  <p style={{ fontSize: 12, color: "#475569", margin: "0 0 2px 0", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 700 }}>
                    Shot list generated for
                  </p>
                  <p style={{ fontSize: 14, color: "#94a3b8", margin: 0 }}>
                    "{generatedFor.length > 80 ? generatedFor.slice(0, 80) + "…" : generatedFor}"
                  </p>
                </div>
                <div style={{
                  padding: "5px 14px", borderRadius: 20,
              ...
              <div style={{
                ...
                {[...new Set(shots.map(s => s.mood))].map(mood => {
                  const style = MOOD_STYLES[mood] || MOOD_STYLES["Expert"];
                  return (
                    <span key={mood} style={{
              ...
              <div style={{
                ...
                <p style={{ fontSize: 16, fontWeight: 700, color: "#a5b4fc", margin: 0 }}>
                  Open each shot, copy the prompt, and generate it in VeraLooks.
                </p>
              </div>
            </div>
          )}
          ...
          {!loading && shots.length === 0 && !error && (
            <div style={{
              ...
              <p style={{ fontSize: 13, color: "#334155", margin: 0 }}>
                Your personalized shot list will appear here
              </p>
            </div>
          )}
