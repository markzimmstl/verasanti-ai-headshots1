...
import { Upload, X, AlertCircle, Check, Info } from 'lucide-react';
...
export const UploadStep: React.FC<UploadStepProps> = ({ 
  ...
  // Internal component to handle drag/drop logic without external library
  const UploadSlot = ({ role, label, subLabel }: { role: keyof MultiReferenceSet, label: string, subLabel?: string }) => {
    ...
    const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files.length > 0) {
        handleFileSelect(e.target.files[0], role);
      }
    };
      ...
      <div className="flex flex-col gap-2">
        ...
        {image ? (
          ...
          >
            ...
            />
            <div className="p-3 bg-slate-800 rounded-full mb-3 text-slate-400 group-hover:text-indigo-400 transition-colors">
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-10">
        {/* Main Photo - Larger & Prominent */}
        <div className="sm:col-span-2 sm:row-span-2 relative">
          <UploadSlot 
            role="main" 
            label="Upload your main facial reference photo"
            subLabel="Clear lighting, looking at camera."
          />
          {/* Tip */}
        ...
        {/* Supporting Photos */}
        <UploadSlot role="sideLeft" label="Side Profile (Left)" subLabel="Optional" />
        <UploadSlot role="sideRight" label="Side Profile (Right)" subLabel="Optional" />
        <UploadSlot role="fullBody" label="Full Body Shot" subLabel="Optional: For body type" />
      </div>
