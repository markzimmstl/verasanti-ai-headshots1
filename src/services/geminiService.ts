...
const buildPrompt = (
  ...
  // 1. Framing Instructions & Negative Constraints
  ...
  let negativeConstraints = `
    ...
    - (Exception: If the scene description specifically asks for a "photo studio set" with visible gear, ignore this constraint).
  `;

  // --- RETOUCH LEVEL LOGIC: TEXTURE PRESERVATION ---
  // If Retouch is NONE, we force high-frequency detail.
  ...
  if (hexMatch) {
    ...
    `;
    // Remove "Environment Only" constraint so it feels like a studio
  ...
  switch (config.framing) {
    case "Headshot":
      // UPDATED: Zoom out logic to fix chopped heads
      let poseInstruction = "";
      if (globalIndex < 2) {
          if (globalIndex % 2 === 0) {
...
export const generateBrandPhotoWithRefs = async (
  ...
  // 2. Add max 2 other refs (Gemini 2.5 limit)
  ...
  // Handle custom background (passed either via config or direct arg)
...
export const generateBrandPhotoWithRefsSafe = async (
  ...
  // 2. AUTO-REFINEMENT CHECK (Boardrooms)
  const lowerPrompt = (stylePrompt + (config.backgroundType || '')).toLowerCase();
  const isBoardroomContext = lowerPrompt.includes('boardroom') || lowerPrompt.includes('meeting room') || lowerPrompt.includes('conference room');
  const userRequestedScreen = lowerPrompt.match(/screen|monitor|tv|projector|display|presentation|slide/);

  if (isBoardroomContext && !userRequestedScreen) {
      console.log("Boardroom detected without screen request. Initiating auto-refinement step to ensure clean background...");
      try {
        ...
        return refinedImage;
          ...
          console.error("Auto-refinement failed, returning initial generation instead:", refinementError);
